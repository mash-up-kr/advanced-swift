...
- 정수 index가 없는 대표적인 collection 구현
  ...
  - We prepend another element to the list by creating a new node, with the next: value set
    ...
    - 이 목록 유형은 흥미로운 속성을 가지고 있습니다 : "영구적"입니다. 노드는 변경 불가능합니다. 일단 생성되면 노드를 변경할 수 없습니다. 다른 element를 consing(목록의 앞에 요소를 추가하는 것)하는 것은 list를 복사하지 않습니다???????. 기존 목록의 맨 앞에 링크 된 새로운 노드를 제공합니다.
